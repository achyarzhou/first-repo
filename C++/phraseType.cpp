    ...
    for (int u = 0; u < P; ++u)
    {
        ...
        for (int t = 0; t < N; ++t)
        {
            ...
            for (int w_iter = 0; w_iter < words.size(); ++w_iter)
            {
                int w = words[w_iter];
                if(useWords == 1)
                    probs[t] *= (nwz[w][t]*1.0 + beta_words)/(nz_words_count[t] + (wordVocabSize*beta_words));
            }
            ...
            for (int sp_iter = 0; sp_iter < sig_phrases.size(); ++sp_iter)
            {
                int sp = sig_phrases[sp_iter];
                if(useSP == 1)
                    probs[t] *= (nspz[sp][t]*1.0 + beta_sp)/(nz_sp_count[t] + (spVocabSize*beta_sp));
            }
...
void outputResult()
{
    ...
    for (int t = 0; t < N; ++t)
    {
        ...
        for (int i = 0; i < wordVocabSize; ++i)
            P_w_given_z[t][i] = (1.0*(nzw[t][i]+ beta_words))/(nz_words_count[t] + wordVocabSize*beta_words);

        for (int i = 0; i < spVocabSize; ++i)
            P_sp_given_z[t][i] = (1.0*(nzsp[t][i]+ beta_sp))/(nz_sp_count[t] + spVocabSize*beta_sp);
